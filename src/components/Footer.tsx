...
export default function Footer({ footerSections }: FooterProps) {
  return (
    <footer className="py-12 px-6 lg:px-8 bg-black border-t border-gray-800">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div>
            ...
            <p className="font-heading text-gray-300 tracking-wider">
              UNDERGROUND FASHION FOR RAVERS, BY RAVERS. FESTIVAL-READY GEAR FOR THE ELECTRONIC GENERATION.
            </p>
          </div>
          {footerSections.map((section) => (
            <div key={section.title}>
              <h4 className="font-heading mb-4 tracking-wider">{section.title}</h4>
              <ul className="space-y-2 text-gray-400">
                {section.links.map((link) => (
                  <li key={link.name}>
                    <a href={link.href} className="font-heading hover:text-white transition-colors tracking-wide">
                      {link.name}
                    </a>
                  </li>
                ))}
          ...
          <div>
            <h4 className="font-heading mb-4 tracking-wider">FOLLOW US</h4>
